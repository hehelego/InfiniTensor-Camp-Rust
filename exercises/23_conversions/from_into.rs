...
impl From<&str> for Person {
    fn from(s: &str) -> Self {
        fn from_non_empty(s: &str) -> Option<String> {
            if s.is_empty() {
                None
            } else {
                Some(String::from(s))
        ...
        fn from_with_err(s: &str) -> Option<Person> {
            let (name, age) = s.split_once(',')?;
            let name = from_non_empty(name)?;
            let age = age.parse::<u8>().ok()?;
            Some(Person { name, age })
        }
        from_with_err(s).unwrap_or_default()
    }
}
